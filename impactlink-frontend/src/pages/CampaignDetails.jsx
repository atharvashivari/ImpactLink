    ...
    <PageTransition className="w-100">
      ...
      <section className="full-width-section bg-dark text-white d-flex align-items-end position-relative" style={{ height: "450px", marginTop: "-2rem" }}>
        <div className="position-absolute w-100 h-100"
          style={{ backgroundImage: `url('${campaign.image || "https://images.unsplash.com/photo-1532629345422-7515f3d16bb6?q=80&w=1470&auto=format&fit=crop"}')`, backgroundSize: "cover", backgroundPosition: "center" }}>
          ...
          <div className="row align-items-end">
            <div className="col-lg-8">
              <m.div variants={heroStagger} initial="hidden" animate="visible">
                <m.span className="badge bg-primary rounded-pill px-3 py-2 mb-3 d-inline-block" variants={fadeUp} style={gpuStyles}>Campaign</m.span>
                <m.h1 className="display-4 fw-bold mb-2" variants={fadeUp} style={gpuStyles}>{campaign.title}</m.h1>
                <m.p className="lead mb-0 text-white-50" variants={fadeUp} style={gpuStyles}>
            ...
            <div className="col-lg-4 d-none d-lg-block">
              <m.div
                className="card bg-white text-dark border-0 shadow rounded-4 p-4 mb-n5 position-relative"
                style={{ bottom: "-40px", ...gpuStyles }}
                variants={fadeRight}
                initial="hidden"
                animate="visible"
              >
                <div className="d-flex align-items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <h6 className="fw-bold mb-0">{campaign.creator?.name || "Anonymous"}</h6>
                  </div>
                </div>
                <div className="mb-4">
                  ...
                  <div className="progress" style={{ height: "8px" }}>
                    <m.div
                      ...
                      style={{ transformOrigin: "left", ...gpuStyles }}
                    ></m.div>
                  </div>
                </div>
                <m.a href={`/donate/${campaign._id}`} className="btn-primary-custom w-100 text-center" {...buttonTap}>Donate Now</m.a>
              </m.div>
            </div>
          </div>
        </div>
          ...
          <div className="col-lg-8">
            <FadeIn>
              <ul className="nav nav-tabs mb-4 border-bottom-0 gap-3">
                <li className="nav-item"><button className="nav-link text-dark fw-bold border-0 bg-transparent px-0 border-bottom border-primary border-3" style={{ borderRadius: 0 }}>Description</button></li>
                <li className="nav-item"><button className="nav-link text-muted fw-medium border-0 bg-transparent px-0 hover-primary">Updates</button></li>
                <li className="nav-item"><button className="nav-link text-muted fw-medium border-0 bg-transparent px-0 hover-primary">Comments</button></li>
              </ul>
              <div className="mb-5">
                <h4 className="fw-bold mb-3">About this campaign</h4>
                <p className="text-muted" style={{ whiteSpace: "pre-wrap", lineHeight: "1.8" }}>{campaign.description}</p>
              </div>
            </FadeIn>
          </div>
              ...
              <div className="custom-card p-4 mb-4">
                ...
                <ul className="list-unstyled mb-0 d-flex flex-column gap-3">
                  <li className="d-flex justify-content-between align-items-center pb-3 border-bottom border-light">
                    ...
                    <span className={`badge rounded-pill ${campaign.status === 'Completed' ? 'bg-success-subtle text-success' : 'bg-primary-subtle text-primary'}`}>{campaign.status || 'Active'}</span>
                  </li>
                  <li className="d-flex justify-content-between align-items-center pb-3 border-bottom border-light">
